...
void VisualizeHitsProcessor::Export3DNeighbors(int nNeighbors, const std::string& filename) {
  ...
  for (const auto& hit : _hits) {
    ...
    hitGeo -> SetLineColor(colors[std::min(int(colors.size())-1, int(double(hit.charge)/maxCharge -> charge*colors.size()))]);//TColor::GetColor(std::min(255, int((hit.charge/1000.)*255)), 0, 0));
